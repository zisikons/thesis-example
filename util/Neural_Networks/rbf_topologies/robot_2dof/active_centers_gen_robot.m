function [active_centers] = active_centers_gen_robot(trajectory_centers,regressor_centers,sigmas,bias,DT,threshold,active_states)
...
function dx = ref_sys(t,x,ref_params)
    dx = zeros(4,1);
    dx(1:2) = ref_plant_2nd_order(t,x([1,2]),ref_params.theta1);
    dx(3:4) = ref_plant_2nd_order(t,x([3,4]),ref_params.theta2);
end
